...
public class EmailService {
    ...
    public boolean sendEmailVerify(EmailDto emailDto)  {
        try{
            Context context = new Context();
            context.setVariable("name",emailDto.getAccount().getFullName());
            context.setVariable("button","Click Here to verify");
            context.setVariable("link",emailDto.getLink());
            context.setVariable("date",emailDto.getCreatedDate());
            ...
            messageHelper.setTo(emailDto.getAccount().getEmail());
            messageHelper.setText(template,true);
            messageHelper.setSubject(emailDto.getSubject());
            javaMailSender.send(message);
            return true;
        }catch(MessagingException e){
            throw new EmailException("Error sending verify email, please check your mail again", HttpStatus.BAD_REQUEST);
        }
    }

    public void sendEmailResetPassword(EmailDto emailDto)  {
        try{
            Context context = new Context();
            context.setVariable("name",emailDto.getAccount().getFullName());
            ...
            throw new EmailException("Error sending email reset password, please check your mail again",HttpStatus.BAD_REQUEST);
        }
    }
